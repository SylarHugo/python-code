...
import csv
#initial_value：冠幅，间距，冠高，面积，宽，长，分段个数,扩展比例,hegyi_ratio，R,B,H,square_dm, r_length,r_width, step_point, percentage,hegyi_ratio
def m_s(initial_value):
    accept_p = numpy.zeros(2)
    #基本参数设置
    R, B, H, S, length, step_point, accept_p[0], accept_p[1], hegyi_ratio = initial_value
    width = length
    V_s = S * H#穿透雨的立方形体积
    R2 = pow(R, 2)
