...
def m_c(initial_value):
    accept_p = numpy.zeros(2)
    #基本参数设置
    R, B, H, S, radius, step_point, accept_p[0], accept_p[1] ,hegyi_ratio = initial_value
    V_s = S * H#穿透雨的立方形体积
    R2 = pow(R, 2)#树冠
    r2 = pow(radius, 2)#水槽
    ...
    for one_dimensional in ucs:  # 第二维
        ...
        circle_dot[0] = one_dimensional[1]  # xa
        circle_dot[1] = one_dimensional[2]  # ya
