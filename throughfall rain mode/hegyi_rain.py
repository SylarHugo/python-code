...
H, R, B, pi, step_point, canot_accept = [150, 30, 10, math.pi, 360, 5]
hegyi_ratio = 7 / 6#扩展比例
#模型基础参数
...
accept_ratio = [R_t * (100 - canot_accept) / 100, R_t * (100 + canot_accept) / 100 ]#体积比容差范围
#模型初始值设置
S_f = numpy.arange(10, 101, 5)#水槽面积范围 单位平方分米
...
for square_dm in S_f:
    ...
    initial_value = [R, B, H, square_dm, radius, step_point, accept_ratio[0], accept_ratio[1], hegyi_ratio]
    circle_accept_point_num[len_s] = hegyi_circle.m_c(initial_value)#圆形水槽：采集穿透雨 >= 95% 全林穿透雨占比的个数
    s_side = numpy.sqrt(square_dm)#正方形边长
    initial_value = [R, B, H, square_dm, s_side, step_point, accept_ratio[0], accept_ratio[1], hegyi_ratio]
    square_accept_point_num[len_s] = hegyi_square.m_s(initial_value)
    #长方形
    len_r = 0
    ...
    for len_l_w in ratio_l_w:#len_rp等面积内，长方形长宽比数组的排序
        ...
        initial_value = [R, B, H, square_dm, r_length, r_width, step_point, accept_ratio[0], accept_ratio[1],
                         hegyi_ratio]
        rectangle_best_S_point[len_r] = hegyi_rectangle.m_r(initial_value)#满足接受条件的点的个数
        if len_r == 0:
